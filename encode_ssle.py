...
Grid capacity (approximate, default 200x200):
    ~4.5 KB per disc at starter density
    ~18 KB at 400x400
    Increase --cols/--rows for larger files.
"""
...
MAGIC = b'5DG\x01'
FIDUCIAL_SIZE = 3       # corner marker size in grid dots
BORDER = 1              # 1-dot clear border inside fiducials
MAX_FILENAME_LEN = 32
...
def encode(input_path, grid_cols, grid_rows, dot_size, dot_spacing, output_path):
    ...
    payload_bits = file_to_bits(header + file_data)

    dcols, drows = data_area(grid_cols, grid_rows)
    capacity_bits = dcols * drows

    if len(payload_bits) > capacity_bits:
        needed = math.ceil(math.sqrt((len(payload_bits) * 1.15) / 1))
        print(f"ERROR: File too large for this grid.")
        print(f"  Capacity : {capacity_bits} bits ({(capacity_bits - HEADER_BYTES * 8) // 8} bytes for data)")
    ...
    # Build logical grid (1 = engrave dot)
    grid = np.zeros((grid_rows, grid_cols), dtype=np.uint8)

    # Fiducial markers at all 4 corners
    margin = 0
    for ry, rx in [
        (margin, margin),
        (margin, grid_cols - FIDUCIAL_SIZE - margin),
        (grid_rows - FIDUCIAL_SIZE - margin, margin),
        (grid_rows - FIDUCIAL_SIZE - margin, grid_cols - FIDUCIAL_SIZE - margin),
    ]:
        grid[ry:ry + FIDUCIAL_SIZE, rx:rx + FIDUCIAL_SIZE] = 1

    # Fill data area row by row
    ...
    fill = len(file_to_bits(header + file_data)) / capacity_bits * 100
    grid_mm = (img_w / 300) * 25.4  # assuming 300 DPI reference

    print(f"✅  Encoded  : {input_path}")
    print(f"    Size     : {file_size} bytes  |  CRC32: {crc32:08X}")
    print(f"    Grid     : {grid_cols}×{grid_rows} dots  |  Data area: {dcols}×{drows}  |  {fill:.1f}% full")
    print(f"    PNG      : {output_path}  ({img_w}×{img_h} px)")
    print()
    print(f"📐  xTool F2 Ultra settings:")
    print(f"    Import   : {output_path}")
    print(f"    Size     : set to {grid_mm:.0f}×{grid_mm:.0f} mm in xTool software")
    ...
    print()
    print(f"    When reading back, use: python3 decode_ssle.py --cols {grid_cols} --rows {grid_rows}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Encode a file to an xTool-ready dot-grid PNG for SSLE glass engraving'
    )
    ...
    encode(args.input_file, args.cols, args.rows, args.dot_size, args.dot_spacing, out)
